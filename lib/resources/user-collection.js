...
function UserCollection(name, options) {
  ...
  this.properties.username.required = false;
    ...
    case 'PUT':
      ...
      if(ctx.query.id || ctx.body.id) {
        this.store.first({username: ctx.body.username}, function (err, u) {
            if(u&&(ctx.body.username!=""&&ctx.body.username!=undefined))
                if(u.id != ctx.query.id)
        ...
        this.store.first({username: ctx.body.username}, function (err, u) {
          if(u&&(ctx.body.username!=""&&ctx.body.username!=undefined)) return ctx.done({errors: {username: 'is already in use'}});
          uc.save(ctx, done);
        });
      }
    break;
